...
export default function ResultView({ slug, encoded }: { slug: string; encoded?: string }) {
  ...
  useEffect(() => {
    ...
    }).catch(() => { if (!saved) setState("missing"); });
  }, [initial, slug]);
  async function drawCard() {
    ...
    if (result.profileImage) try { const image = await loadImage(result.profileImage); ctx.save(); ctx.beginPath(); ctx.arc(920, 280, 95, 0, Math.PI * 2); ctx.clip(); const scale = Math.max(190 / image.width, 190 / image.height); ctx.drawImage(image, 920 - image.width * scale / 2, 280 - image.height * scale / 2, image.width * scale, image.height * scale); ctx.restore(); } catch { /* The card is still usable without a profile photo. */ }
  ...
  return <main className="result-page"><nav><a className="brand" href="/"><Mark />WorthMe<span>TM</span></a><a className="nav-link" href="/">Make your own</a></nav><section className="result-wrap"><p className="eyebrow">YOUR ENTERTAINMENT VALUATION</p><h1 className="result-title">{result.name},<br /><em>this is your moment.</em></h1><div className="result-grid"><article className="result-card"><div className="card-top"><span>WORTH SCORE</span><p>For entertainment only</p></div><div className="result-identity">{result.profileImage && <img className="result-profile" src={result.profileImage} alt={`${result.name}'s selected profile`} />}<div><div className="score-line"><strong>{result.score}</strong><span>/100</span></div><p className="valuation">{result.valuation}</p><p className="valuation-label">ENTERTAINMENT VALUATION</p></div></div><div className="human"><span>Human type</span><h2>{result.humanType}</h2></div><div className="bars">{result.scores.map((score) => <div className="bar" key={score.label}><span>{score.label}</span><i><b style={{ width: `${score.value}%` }} /></i><strong>{score.value}</strong></div>)}</div></article><aside className="verdict"><p className="eyebrow">THE VERDICT</p><blockquote>&ldquo;{result.verdict}&rdquo;</blockquote><div className="gap"><span>Your biggest value gap</span><strong>{result.valueGap}</strong><p>{result.upgrade}</p></div></aside></div><div className="result-actions"><button className="button" onClick={download}>Download my card <span aria-hidden="true">down</span></button><button className="button secondary" onClick={share}>Challenge a friend <span aria-hidden="true">share</span></button></div>{notice && <p className="notice" aria-live="polite">{notice}</p>}<p className="disclaimer">WorthMe is a playful entertainment experience - not a financial, professional, employment, or psychological assessment. You choose whether to share this card.</p></section><canvas ref={canvas} className="sr-only" aria-hidden="true" /></main>;
}