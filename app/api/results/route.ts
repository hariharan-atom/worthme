...
export async function POST(request: Request) {
  ...
  if (!isValidAnswers(input)) return NextResponse.json({ error: "Please complete every field, use a valid profile photo, and confirm you are 18+." }, { status: 400 });
  ...
  return NextResponse.json({ result, stored: Boolean(supabase) });
}